...
function ConfigManager:SetupConfigUI(tab, Avantrix)
    ...
    -- Restore Library AutoSave state if saved
    local savedAutoSave = AutoloadConfig:Get("library_autosave")
    if savedAutoSave ~= nil and ConfigManager.Window then
        if ConfigManager.Window.SetAutoSave then
            ConfigManager.Window:SetAutoSave(savedAutoSave)
        else
            ConfigManager.Window.AutoSave = savedAutoSave
        end
    end
