...
function Renamer.beautify(code, options)
    ...
    for varName in code:gmatch("local%s+([a-z])%s*[=,;%s]") do
        if not variables[varName] then
    ...
    for varName in code:gmatch("for%s+([a-z])%s+in") do
        if not variables[varName] then
            variables[varName] = true
        end
    end
    ...
    for varName, _ in pairs(variables) do
        ...
        local firstPos = code:find("[^%w_]" .. varName .. "[^%w_]") or code:find("^" .. varName .. "[^%w_]") or code:find("[^%w_]" .. varName .. "$")
        if firstPos or code:find(varName) then
            local baseName = Renamer.analyzeContext(code, varName, firstPos or code:find(varName) or 1)
    ...
    for _, var in ipairs(sortedVars) do
        local oldEscaped = var.old:gsub("([%^$()%.[%]*+%-?])", "%%%1")

        -- Pattern 1: Variable surrounded by non-word characters
        result = result:gsub("([^%w_])" .. oldEscaped .. "([^%w_])", "%1" .. var.new .. "%2")

        -- Pattern 2: Variable at the start (beginning of string or after newline)
        result = result:gsub("^" .. oldEscaped .. "([^%w_])", var.new .. "%1")
        result = result:gsub("\n" .. oldEscaped .. "([^%w_])", "\n" .. var.new .. "%1")

        -- Pattern 3: Variable at the end (end of string or before newline)
        result = result:gsub("([^%w_])" .. oldEscaped .. "$", "%1" .. var.new)
        result = result:gsub("([^%w_])" .. oldEscaped .. "\n", "%1" .. var.new .. "\n")

        -- Pattern 4: Variable standing alone (whole line or statement)
        result = result:gsub("([%s(,;={])" .. oldEscaped .. "([%s)%],;=}])", "%1" .. var.new .. "%2")
    end
