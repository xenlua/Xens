...
local ConfigManager = {
    ...
    Parser = {
        ...
        Toggle = {
            Save = function(obj)
                local value = obj.Value
                if value == nil then value = obj.State end
            ...
            for i, element in ipairs(ConfigManager.Window.AllElements) do
                if element and element.__type and ConfigManager.Parser[element.__type] then
                    if element.Title and not ConfigManager.ExcludedTitles[element.Title] then
                        ...
                        local current = element.Instance or element.Frame
                        while current and current ~= game do
                            if current:IsA("ScrollingFrame") and current.Name == "Elements" then
                                -- Ini adalah container tab, cari nama tabnya
                                if current.Parent and current.Parent:FindFirstChild("Title") then
                                    tabName = current.Parent.Title.Text
                                end
                                break
                            end
    ...
    if ConfigManager.Window then
        ...
        libraryAutoSaveToggle = tab:Toggle({
            ...
            Callback = function(state)
                ...
                -- Save state to autoload config permanently
